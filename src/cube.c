...
static void demo_draw_build_cmd(struct demo *demo, VkCommandBuffer cmd_buf) 
{
    ...
    memset(&label, 0, sizeof(VkDebugUtilsLabelEXT));
    memset(&cmd_buf_info, 0, sizeof(VkCommandBufferBeginInfo));
    memset(clear_values, 0, sizeof(VkClearValue));
    memset(&rp_begin, 0, sizeof(VkRenderPassBeginInfo));
    memset(&cmd_buf_name, 0, sizeof(VkDebugUtilsObjectNameInfoEXT));
    memset(&viewport, 0, sizeof(VkViewport));
    memset(&scissor, 0, sizeof(VkRect2D));
    memset(&image_ownership_barrier, 0, sizeof(VkImageMemoryBarrier));
...
*/
        ...
        cmd_buf_name.pNext = NULL;
        cmd_buf_name.objectType = VK_OBJECT_TYPE_COMMAND_BUFFER;
        /*cmd_buf_name.objectHandle = (uint64_t)cmd_buf;
        */cmd_buf_name.objectHandle = (uint64_t)(uintptr_t)cmd_buf;
        cmd_buf_name.pObjectName = "CubeDrawCommandBuf";
...
static void demo_draw(struct demo *demo) 
{
    ...
    do 
    {
        ...
        err = demo->fpAcquireNextImageKHR(demo->device, 
                                          demo->swapchain, 
                                          UINT64_MAX,
...
*/
    ...
    present.pWaitSemaphores = (demo->separate_present_queue) ? &demo->image_ownership_semaphores[demo->frame_index]
                                                         : &demo->draw_complete_semaphores[demo->frame_index];
    ...
    if (demo->separate_present_queue) 
    {
        present.pWaitSemaphores = &demo->image_ownership_semaphores[demo->frame_index];
    } 
    else 
    {
        present.pWaitSemaphores = &demo->draw_complete_semaphores[demo->frame_index];
    }
...
static void demo_prepare_buffers(struct demo *demo) 
{
    ...
    if (surfCapabilities.maxImageExtent.width  == 0 || 
    ...
    */
    ...
    for (i = 0; i < ARRAY_SIZE(compositeAlphaFlags); i++) 
    {
...
*/
    ...
    */
    ...
    swapchainImages = (VkImage *)malloc(demo->swapchainImageCount * sizeof(VkImage));
...
static void demo_prepare_texture_buffer(struct demo *demo, 
                                        const char *filename, 
...
*/
        ...
        buffer_create_info.size = tex_width * tex_height * 4;
...
static void demo_prepare_textures(struct demo *demo) 
{
    ...
    for (i = 0; i < DEMO_TEXTURE_COUNT; i++) 
    {
        ...
        if ((props.linearTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT) && !demo->use_staging_buffer) 
        {
            ...
            demo_prepare_texture_image(demo, 
                                        tex_files[i], 
                                        &demo->textures[i], 
...
*/
            ...
            copy_region.bufferRowLength = demo->staging_texture.tex_width;
            copy_region.bufferImageHeight = demo->staging_texture.tex_height;
            ...
            copy_region.imageExtent.width = demo->staging_texture.tex_width;
            copy_region.imageExtent.height = demo->staging_texture.tex_height;
...
static void demo_prepare_descriptor_set(struct demo *demo) 
{
    ...
    for (i = 0; i < demo->swapchainImageCount; i++) 
    {
        err = vkAllocateDescriptorSets(demo->device, 
                                       &alloc_info, 
...
static void demo_prepare_framebuffers(struct demo *demo) 
{
    ...
    for (i = 0; i < demo->swapchainImageCount; i++) 
    {
        ...
        err = vkCreateFramebuffer(demo->device, 
                                  &fb_info, 
                                  NULL, 
...
static void demo_cleanup(struct demo *demo) 
{
    ...
    {
        vkWaitForFences(demo->device, 
                        1, 
                        &demo->fences[i], 
                        VK_TRUE, 
                        UINT64_MAX);
        vkDestroyFence(demo->device, 
                       demo->fences[i], 
                       NULL);
    ...
    */
    if (!demo->is_minimized) 
    {
        ...
        vkDestroyDescriptorPool(demo->device, 
                                demo->desc_pool, 
                                NULL);

        vkDestroyPipeline(demo->device, 
                          demo->pipeline, 
                          NULL);
        vkDestroyPipelineCache(demo->device, 
                               demo->pipelineCache, 
                               NULL);
        vkDestroyRenderPass(demo->device, 
                            demo->render_pass, 
                            NULL);
        vkDestroyPipelineLayout(demo->device, 
                                demo->pipeline_layout, 
                                NULL);
        vkDestroyDescriptorSetLayout(demo->device, 
                                     demo->desc_layout, 
                                     NULL);
        ...
        for (i = 0; i < DEMO_TEXTURE_COUNT; i++) 
        {
            vkDestroyImageView(demo->device, 
                               ...
                               NULL);
            vkDestroyImage(demo->device, 
                           demo->textures[i].image, 
                           NULL);
            vkFreeMemory(demo->device, 
                         demo->textures[i].mem, 
                         NULL);
            vkDestroySampler(demo->device, 
                             demo->textures[i].sampler, 
                             NULL);
        }
        demo->fpDestroySwapchainKHR(demo->device, 
                                    demo->swapchain, 
                                    NULL);
        ...
        for (i = 0; i < demo->swapchainImageCount; i++) 
        {
            vkDestroyImageView(demo->device, 
                               ...
                               NULL);
            vkFreeCommandBuffers(demo->device, 
                                 demo->cmd_pool, 
                                 1, 
        ...
        vkDestroyCommandPool(demo->device, 
                             demo->cmd_pool, 
                             NULL);
    ...
    if (demo->validate) 
    {
        demo->DestroyDebugUtilsMessengerEXT(demo->inst, 
                                            demo->dbg_messenger, 
                                            NULL);
    }
    vkDestroySurfaceKHR(demo->inst, 
                        demo->surface, 
                        NULL);
...
static void demo_create_xlib_window(struct demo *demo) 
{
    ...
    demo->xlib_window = XCreateWindow(demo->display, 
                                      RootWindow(demo->display, 
                                      vInfoTemplate.screen), 
                                      0, 
                                      0, 
                                      demo->width,
                                      demo->height, 
...
static void handle_toplevel_configure(void *data, 
                                      struct xdg_toplevel *xdg_toplevel UNUSED, 
                                      int32_t width, 
                                      int32_t height,
...
static void demo_handle_directfb_event(struct demo *demo, 
...
static VkSurfaceFormatKHR pick_surface_format(const VkSurfaceFormatKHR *surfaceFormats, 
                                              uint32_t count) 
{
    uint32_t i;
...
static void demo_init_vk_swapchain(struct demo *demo) 
{
    ...
    supportsPresent = (VkBool32 *)malloc(demo->queue_family_count * sizeof(VkBool32));
                                  ...
                                  uint32_t          state) 
{
    ...
    {
        xdg_toplevel_move(demo->xdg_toplevel, 
                          demo->seat, 
                          serial);
    }
}
                                     ...
                                     enum wl_seat_capability caps) 
{
    ...
    if (caps & WL_SEAT_CAPABILITY_KEYBOARD) 
    {
        ...
        wl_keyboard_add_listener(demo->keyboard, 
                                 &keyboard_listener, 
                                 demo);
    } 
...
static void wm_base_ping(void *data UNUSED, 
                         struct xdg_wm_base *xdg_wm_base, 
                         uint32_t serial) 
{
                                   ...
                                   uint32_t version UNUSED) 
{
    ...
    if (strcmp(interface, wl_compositor_interface.name) == 0) 
    {
        ...
        demo->compositor = wl_registry_bind(registry, 
                                            id, 
    ...
    {
        demo->xdg_wm_base = wl_registry_bind(registry, 
                                             id, 
...
static void demo_init(struct demo *demo, int argc, char **argv) 
{
    ...
    mat4x4_perspective(demo->projection_matrix, 
                       (float)degreesToRadians(45.0f), 
                       1.0f, 
                       0.1f, 
                       100.0f);
    mat4x4_look_at(demo->view_matrix, 
                   eye, 
                   origin, 
                   up);
        ...
        case APP_CMD_INIT_WINDOW: 
            ...
            if (app->window) 
            {
                ...
                */
                ...
                for (int i = 0; i < argc; i++) __android_log_print(ANDROID_LOG_INFO, 
                                                                   appTag, 
                                                                   "argv[%i] = %s", 
                                                                   i, 
                                                                   argv[i]);
